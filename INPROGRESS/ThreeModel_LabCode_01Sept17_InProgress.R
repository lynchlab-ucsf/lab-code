...
pacman::p_load(pscl,MASS,foreach,doMC)

rm(list=ls())

workdir <- "/data/Users/kmccauley/WaldStudy/Analysis/ThreeModel" #Directory where you want to save data
otutable <- "../../rarefied_filtered_37692_idupdate.txt" #OTU table in text format with "taxonomy" as a column at some point (not necessarily at the end)
mapfile <- "../../Final_Map_Wald_AllSummaryData.txt" #Map file
treatment <- "Visit.Type_Lab" #Variable (from map file) of comparison
trt1 <- "Surveillance/Entry" #Reference group in this analysis
trt2 <- "Sick - Acute - Sinusitis"
outputname <- "Sinusitis_Healthy.csv" #CSV filename where results will be sent to
imagename <- "rawdata_wald" #Because computation time can be extensive, and the model-selection step can break, I save the intermediate working enviornment under this name to be able to pull it up and troubleshoot after the parallel part
...
ss_var <- "dust_data" # Which *variable* determines the subsetting (in the above example, "month")
ss_group <- "TRUE" # The specific group that you want results for (here, "June")

mixed_effects <- TRUE #Do you want to run a mixed-effects (ME) analysis (ie, correlated samples)
ind_id <- "PatID" #Individual ID: Set to NULL if not using ME models
...
if(class(try(pacman::p_load(glmmTMB,lme4))) == "try-error") {
  print("Installing glmmTMB")
  devtools::install_github("glmmTMB/glmmTMB/glmmTMB")
  pacman::p_laod(glmmTMB, lme4)
} else {
 pacman::p_load(glmmTMB, lme4)
}
}
...
if(mixed_effects == TRUE) {
  ...
  if(drop_warnings == TRUE) {
    result.pois <- tryCatch(glmmTMB(formula1, family=poisson(link='log'),ziformula=~0, data = both),error=function(e) NA)
    result.nb <- tryCatch(glmmTMB(formula1, family=nbinom2(link='log'), data = both,ziformula=~0),error=function(e) NA)
    result.zinb <- tryCatch(glmmTMB(formula1, data = both,family=nbinom2(link='log'),ziformula=~1),error=function(e) NA)
  } else {
    pois <- myTryCatch(glmmTMB(formula1, family=poisson(link='log'),ziformula=~0, data = both))
    nb <- myTryCatch(glmmTMB(formula1, data = both,family=nbinom2(link='log'),ziformula=~0))
    zinb <- myTryCatch(glmmTMB(formula1, data = both,family=nbinom2(link='log'),ziformula=~1))
  ...
  nb.disp <- tryCatch(getME(result.nb,"theta"), error=function(e) NA) # Negative-Binomial dispersion parameter (from NB model)
  zinb.disp  <- tryCatch(getME(result.zinb,"theta"),error=function(e) NA) #Negative-Binomial dispersion parameter (from ZINB model)
...
if(mixed_effects == TRUE) {
  if(drop_warnings == TRUE) {
  finaldata <- c(OTUname, pois.coeff,pois.pval,nb.coeff,nb.pval,zinb.coeff,zinb.pval,mean_trt1,mean_trt2,mean_diff,wgt_mean_trt1,wgt_mean_trt2,wgt_mean_diff,aic.pois,aic.nb,aic.zinb,bic.pois,bic.nb,bic.zinb,nb.disp,zinb.disp,zerotrt1,zerotrt2, nonzerotrt1, nonzerotrt2, totaltrt1, totaltrt2,variance)
  } else {
  finaldata <- c(OTUname, pois.coeff,pois.pval,nb.coeff,nb.pval,zinb.coeff,zinb.pval,mean_trt1,mean_trt2,mean_diff,wgt_mean_trt1,wgt_mean_trt2,wgt_mean_diff,aic.pois,aic.nb,aic.zinb,bic.pois,bic.nb,bic.zinb,nb.disp,zinb.disp,zerotrt1,zerotrt2, nonzerotrt1, nonzerotrt2, totaltrt1, totaltrt2,variance,error.pois,error.nb,error.zinb,warn.pois,warn.nb,warn.zinb)
  }
} else {
  if(drop_warnings == TRUE) {
...
if(mixed_effects == TRUE) {
   if(drop_warnings == TRUE) {
names(all_data) <- c("OTUname","pois.coeff","pois.pval","nb.coeff","nb.pval","zinb.coeff","zinb.pval","mean_trt1","mean_trt2","mean_diff","wgt_mean_trt1","wgt_mean_trt2","wgt_mean_diff","aic.pois","aic.nb","aic.zinb","bic.pois","bic.nb","bic.zinb","nb.disp","zinb.disp","zerotrt1","zerotrt2","nonzerotrt1","nonzerotrt2","totaltrt1","totaltrt2","variance")
   } else {
