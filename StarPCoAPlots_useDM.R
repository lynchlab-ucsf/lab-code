...
star.figs <- function(dm, variable, dat, mypalette=NULL, sampid=0) {
  ...
  if(!is.null(mypalette)) {
    ...
    tempPal <- gg_color_hue(length(levels(factor(dat[,variable]))))
    names(tempPal) <- levels(factor(dat[,variable]))
  }
  if(sampid == 0) {
  ...
  myplot <- ggplot(gg, aes(x=Axis.1, y=Axis.2, color=gg[,variable])) +
    ...
    geom_point(aes(x=Axis.1.centroid, y=Axis.2.centroid), size=3, shape=21,fill="white", stroke=4, show.legend=FALSE) +
    stat_ellipse(type="norm", level=0.68)
  print(myplot)
}

#star.figs(bray.dm, "dom_genus2", dat=mapping.pheno.1yr, sampid="#SampleID")
