...
########################################################################
#install.packages("glmmADMB","nlme","doMC","foreach","MASS","pscl")
...
rm(list=ls())
workdir <- "/data/Users/kmccauley/PROSE_NEW/AnalysisData/" #Directory where you want to save data
otutable <- "/data/Users/kmccauley/PROSE_NEW/AnalysisData/PROSE_OTUtable_June.txt" #OTU table in text file from BIOM format with "taxonomy" as a column at some point (not necessarily at the end)
mapfile <- "/data/Users/kmccauley/PROSE_NEW/AnalysisData/FINAL_MAP_FILE.txt" #Map file, typically in QIIME format, with "#SampleID" as the first variable
treatment <- "exac" #Variable (from map file) of comparison
trt1 <- "0" #Reference group in this analysis
trt2 <- "1"
outputname <- "TESTING_drop_ThreeModel.csv" #CSV filename where results will be sent to
imagename <- "rawdata_testing" #Because computation time can be extensive, and the model-selection step can break, I save the intermediate working enviornment under this name to be able to pull it up and troubleshoot after the parallel part
subset <- TRUE # Do you want to perform subset analysis? In the prose analysis, I wanted results among Placebo, ICS, and Xolair separately
...
skip_first <- FALSE #Skip the first line when reading in your OTU table
...
matrix1 <- MYdata[c(as.character(MYmeta[,"#SampleID"][matrix1.subset]))] # pull out only the SampleIDs that have data on your variable of interest, based on T/F
...
matrix2 <- MYdata[c(as.character(MYmeta[,"#SampleID"][matrix2.subset]))] # pull out only the SampleIDs that have data on your variable of interest, based on T/F
...
MYmeta2 <- MYmeta[,c("#SampleID",ind_id,treatment)] #The non-OTU metadata needed for the upcoming loop
both <- merge(mat.array,MYmeta2,by.x=0,by.y="#SampleID") #Add to OTU data
both[,ind_id] <- factor(as.character(both[,ind_id])) #LME models need studyid to be a factor
} else {
MYmeta2 <- MYmeta[,c("#SampleID",treatment)] #The non-OTU metadata needed for the upcoming loop
both <- merge(mat.array,MYmeta2,by.x=0,by.y="#SampleID") #Add to OTU data
}
...
all_data <- foreach(i=1:(length(rownames(MYdata))),.combine='rbind',.packages=c('lme4','glmmADMB')) %dopar% {
